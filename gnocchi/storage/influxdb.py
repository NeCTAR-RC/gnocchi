...
OPTS = [
    ...
    cfg.BoolOpt('influxdb_auto_create_db',
                default=True,
                help='Create the influxDB database on driver initialisation'),
...
class InfluxDBStorage(storage.StorageDriver):
    ...
    def backfill_data(self):
        ...
        result = list(result[self.raw_data_measurment])
        if not result:
            return
        start_time = result[0]['time']
    ...
    def process_background_tasks(self, index, block_size=128, sync=True):
        self.backfill_data()
    ...
    def _make_time_query(self, from_timestamp, to_timestamp, granularity):
        if from_timestamp:
            from_timestamp = find_nearest_stable_point(from_timestamp,
                                                       granularity)
            left_time = "time >= '%s'" % self._timestamp_to_utc(
